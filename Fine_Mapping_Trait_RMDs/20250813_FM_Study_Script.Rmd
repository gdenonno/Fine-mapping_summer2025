...
## 1. Select study and gather info
...
Get RUN_FM_TABLE index for study
```{r}
run_fm_index <- match(Study, run_FM_table$Study)
```
...
chmod +x ${main_directory}slurm_scripts/MungeSumstats_flipfrq_slurm.sh # Making sure slurm script is executable. (Only need to do once)
sbatch ${main_directory}slurm_scripts/MungeSumstats_flipfrq_slurm.sh # Submit Slurm job 
scontrol show job 69161
```
...
## 6. Check if all variants in high LD with lead SNPs are present 

### 6a. Submit Slurm script subset_1000g_for_LD_check.sh to subset VCFs to 100,000 bp around lead SNPs
```{r}
...
### 6b. Make plink files for the subsetted 1000 genomes vcfs based on ancestry samples
```{bash}
main_directory="$HOME/fine_mapping/"
...
### 6c. Create a file that lists all the positions of variants in high LD (r2 >= 0.6)
```{bash}
while read -r rsid chr pos19; do
...
### 6d. Cutting loci that do not have all SNPs in high LD in the GWAS (with the exception of lead SNPs missing high LD indels). This is because all indels have been cut from the GWAS. 
```{r}
rows_to_drop <- c()
...
### 7a. Submit slurm script VCFS_plink_LD_slurm.sh to cut VCF to GWAS subset positions, while also filtering by maf > 0.01. 


### 7b-d. Make plink files with cut VCFs. Filter out low maf, non-biallelic SNPs, and samples that aren't in ancestry. Create LD matrix and frq files. 
```{bash}
main_directory="$HOME/fine_mapping/"
cd ${main_directory}FM_results/${Trait}/${Trait}_${Study}_${Ancestry}
...
## 8. Align the GWAS and bim file
Align the GWAS subset with the bim file to avoid dimensional errors between LD matrix and GWAS subset. Save aligned GWAS subset and the info it contains for study SNPs.
```{r}
for(i in 1:nrow(LD_filtered_gwas_hits)){
  ...
  for(j in 1:nrow(gwas_subset)){
    ...
    bp_38 <- gwas_subset$HM_POS[j] # CHANGE DEPENDING ON GWAS COL NAMES
    variant <- gwas_subset$HM_VARIANT_ID[j] # CHANGE DEPENDING ON GWAS COL NAMES
    # variant <- paste0(chr, "_", bp_38, "_", ref, "_", alt) # CHANGE DEPENDING ON GWAS COL NAMES
...
for(i in 1:nrow(LD_filtered_gwas_hits)){
  ...
  write.table(qc_table, file = paste0(working.dir, locus, "/", locus, "_", Ancestry, "_qc_table.tsv"),
...
write.table(all_loci_data, paste0(main.dir, "FM_all_loci_data_tables/", Trait, "_", Study, "_", Ancestry, "_all_loci_data.tsv"), col.names = TRUE, row.names = F, sep = "\t")
```

## 10.QC Fix

Beginning of QC Fix and trying to fix problem SNPs. 
```{r}
all_loci_data <- fread(paste0(main.dir, "FM_all_loci_data_tables/", Trait, "_", Study, "_", Ancestry, "_all_loci_data.tsv"), header = F, sep="\t")
# colnames(all_loci_data) <- c("TRAIT", "STUDY", "LOCUS", "CS_BPS", "SNP", "CHR", "BP", "REF", "ALT", "MAF", "PVAL", "PIP", "CS", "CS_SIZE", "LD_to_LEAD", "LAMBDA", "BP_38", "VARIANT")
colnames(all_loci_data) <- c("V1", "TRAIT", "STUDY", "LOCUS", "CS_BPS", "SNP", "CHR", "BP", "REF", "ALT", "MAF", "PVAL", "PIP", "CS", "CS_SIZE", "LD_to_LEAD", "LAMBDA", "VARIANT")
```

```{r}
loci_to_fix <- c()
fence <- with(all_loci_data, quantile(LAMBDA, 0.75, na.rm = TRUE) + 1.5 * IQR(LAMBDA, na.rm = TRUE))

for(i in 1:nrow(LD_filtered_gwas_hits)){
  locus <- LD_filtered_gwas_hits$SNP[i]
  lambda_indx <- which(all_loci_data$LOCUS == locus)[1]
  if(all_loci_data$LAMBDA[lambda_indx] > 0.1 || all_loci_data$LAMBDA[lambda_indx] > fence){
    loci_to_fix <- c(loci_to_fix, locus)
  }
}

for (locus in loci_to_fix) {
  qc_table <- fread(paste0(working.dir, locus, "/", locus, "_", Ancestry, "_qc_table.tsv"))
  #colnames(qc_table) <- c("z", "condmean", "condvar", "z_std_diff", "logLR")
  
  snp_list_df <- fread(paste0(working.dir, locus, "/", locus, 
                                  ".locus.", Ancestry, ".matrix.snplist"), header = FALSE)
  snp_list <- snp_list_df[[1]]
  
  qc_table <- cbind(snp = snp_list, qc_table)
